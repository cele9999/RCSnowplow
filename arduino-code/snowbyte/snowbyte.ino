...
int sidectrl, fwdctrl, trimctrl, leverctrl;  //Stores RF Receiver values
...
const int ch3 = 13;    // --> D13         --> INPUT  --> channel 3 of RF Receiver
...
const int trimmer = A0;// --> D14      --> OUTPUT --> Trimmer Motor On/Off
...
void loop() {
  ...
  //TRIMMER ON/OFF
   if (sidectrl == 1000 && fwdctrl == 1000 && trimctrl == 1000 && leverctrl == 1000 && control == 0) {
    Serial.println("No controller detected");
  } else if (trimctrl >= 1600) {
    //TRIMMER ON
    Serial.println("ch3=~2000");
    control = 1;
    digitalWrite(trimmer, HIGH);
    //TRIMMER OFF
  } else {
    Serial.println("ch3=~1000");
    digitalWrite(trimmer, LOW);
  }
